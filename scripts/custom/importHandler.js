...
function register(headers, importData, notificationCallBack) {
  ...
  function getTEIByAttr(rootOU, attr, value, lookUpIndex, ou) {
    ...
    $.ajax({
      ...
      async: false,
      url:
        "../../trackedEntityInstances?ou=" +
        rootOU +
        "&ouMode=DESCENDANTS&filter=" +
        attr +
        ":eq:" +
        value,
  ...
  function enroll(index, data, headers, tei, teiHeaderKey) {
    ...
    enrollment.POST(
      enrollmentSuccessCallback,
      enrollmentFailedCallback,
      index,
      headers
    );
  ...
  function createEvent(header, index) {
    ...
    for (var key in headers) {
      ...
      if (/ev\d{1,}/.test(domain)) {
        let event = createSingleEvent(headers[key], tei, program, index);
        if (event.eventDate) events.push(event);
      }
    }
...
function importHandler(headers, importData, notificationCallback) {
  ...
  function importEvent(index, data, header, lookUpFlag, lookUpIndex, tei) {
    ...
    if (lookUpFlag) {
      getTEIByAttr(
        ROOT_OU_UID,
        header[lookUpIndex].args,
        data[index][header[lookUpIndex].key]
      ).then(function (tei) {
        var event = new dhis2API.event();
  ...
  function importTEI(index, data, header, lookUpFlag, lookUpIndex) {
    ...
    if (lookUpFlag) {
      ...
      getOuByCode(code, lookUpIndex).then(function (orgUnits) {
        ...
        getTEIByAttr(
          ROOT_OU_UID,
          header[lookUpIndex2].args,
          data[index][header[lookUpIndex2].key],
          lookUpIndex,
          orgUnit
        ).then(function (data_tei) {
          ...
          if (teiSecondComingMap[DOMAIN_TEI + response.importStat.index]) {
            ...
            getEnrollmentByTei(
              ...
              if (enrollments.length > 0) {
                ...
                response.teiSecondComing =
                  teiSecondComingMap[DOMAIN_TEI + response.importStat.index];
    ...
    if (lookUpFlag) {
      getTEIByAttr(
        ...
        if (data_tei.trackedEntityInstances.length != 0) {
          ...
          var tei = new dhis2API.trackedEntityInstance(
            data_tei.trackedEntityInstances[0]
          );
  ...
  function enrollTEI(index, importData, header, lookUpFlag, lookUpIndex) {
    ...
    if (lookUpFlag) {
      getTEIByAttr(
        ROOT_OU_UID,
        header[lookUpIndex].args,
        importData[index][header[lookUpIndex].key]
      ).then(function (tei) {
        if (tei.length == 0) {
          var response = {};
          response.importStat = {};
        ...
        var enrollment = new dhis2API.enrollment();
        enrollment.excelImportPopulator(
          header,
          importData[index],
          tei[0].trackedEntityInstance
        );
  ...
  function getOuByName(name, level, parentUID) {
    ...
    $.ajax({
      ...
      url:
        ...
        level +
        "&fields=id,name,parent,shortName,openingDate&filter=parent.id:eq:" +
        parentUID +
        "&filter=name:eq:" +
        name,
      success: function (data) {
  ...
  function getEventByTei(teiUid, psUid) {
    ...
    $.ajax({
      ...
      url:
        ...
        teiUid +
        "&order:lastUpdated:asc&pageSize=1&fields=event",
      success: function (data) {
        def.resolve(data);
      },
    });
  ...
  function isTrackerSingleSheetCase() {
    if (headers.length == 2) {
      if (
        headers[0][0].domain == DOMAIN_TEI &&
        headers[1][0].domain == DOMAIN_EVENT
      ) {
        return true;
      }
    }
